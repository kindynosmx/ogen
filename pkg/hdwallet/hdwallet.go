...
func PrivateKeyFromSeedAndPath(seed []byte, path string) (common.SecretKey, error) {
	...
	for i := range pathBits {
		...
		if pathBits[i] == "m" {
			...
			if err != nil {
				return nil, errors.New("failed to generate master key at path component")
			}
		} else {
			if i == 0 {
			...
			if err != nil {
				return nil, errors.New("failed to derive child SK at path component")
			}
		}
	}
	...
	return bls.SecretKeyFromBytes(bytes)
}
...
func parentSKToLamportPK(parentSK *big.Int, index uint32) ([]byte, error) {
	...
	for i := 0; i < 255; i++ {
		copy(lamportPK[32*i:], chainhash.HashB(lamport0[i][:]))
	}
	for i := 0; i < 255; i++ {
		copy(lamportPK[(i+255)*32:], chainhash.HashB(lamport1[i][:]))
	}
	compressedLamportPK := chainhash.HashB(lamportPK)
